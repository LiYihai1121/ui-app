...
自 v3.1 起，版本序列以 [ROADMAP-ADS.md](ROADMAP-ADS.md) 里程碑为准（避免两份文档各自承诺同一版本号）：

- [ ] **v3.1（M1）**：L1 无障碍引擎增强——选择器引擎 + 快照工具（技术方案见 [DESIGN-PHASE1-SELECTOR.md](DESIGN-PHASE1-SELECTOR.md)）
- [ ] **v3.2（M2）**：L2 网络过滤层——DNS 过滤 + `filter-rules` 路由
- [ ] **v3.3（M3）**：L3 防摇一摇模块（独立可选 APK）
- [ ] **v4.0（M4）**：L4 通知过滤 + ROM 指引 + 规则生态工具
